...
class road_tracking():
    ...
    root_dir='data/'
    model_dir = 'weights/'
    save_topology_root = 'results/output/'
    checkname = 'weights.pth'

    def __init__(self, imagename):

        self.net = self.build_model()
        self.img_filename = imagename
        self.image = Image.open(os.path.join(self.root_dir, imagename))
        ...
        self.step_size = 20
        self.offset = 0
        self.confidence_th = 0.05
        self.auto_seed_confidence_th = 0.15
        self.connection_th = 10
        self.close_visited_th = 1

        self.visualize_step = False
        self.use_line_conn = True
        self.debug = False
        self.auto_init_seed = False

        self.visualize_tracing = True
        self.visualize_tracing_fig = None # the figure we need to visualize the tracing progress.

        self.seed_stack = SeedStack()
        self.pred_centers = []
        self.turn_direction = False
        self.close_visited_time = 0
        self.valid_seed_count = 0
        self.run_time_with_manual = 0
        self.run_time_without_manual = 0
    ...
    def build_model(self):
        ...
        net.load_state_dict(torch.load(os.path.join(self.model_dir, self.checkname),
    ...
    def save_results_fun(self):
        if not os.path.exists(self.save_topology_root):
            os.makedirs(self.save_topology_root)

        filename, _ = os.path.splitext(self.img_filename)

        img = Image.open(os.path.join(self.root_dir, self.img_filename))
        img = img.convert('RGBA')

        w, h = img.size

        path_mask = np.zeros((h, w, 4), dtype=np.uint8)
        offset = 2
        for path_t in self.visited_path:
            for pt in path_t:
                path_mask[pt[0]-offset:pt[0]+offset+1,pt[1]-offset:pt[1]+offset+1] = (255, 0, 0, 200)

        img_mask = Image.fromarray(path_mask, mode='RGBA')
        ...
        # save the coords of all the pathes
        np.save(os.path.join(self.save_topology_root, filename + '_path.npy'), np.array(self.visited_path))
    ...
    def next_iter(self):
        ...
        while next_center is None:
            ...
            if dist[0]<=self.close_visited_th:
                print('the seed ', seed, 'has been tracked') if self.debug else 1
            else:
        ...
        patch = Patch(box_top, box_left, np_image_crop)
        # predict the road center points, the corresponding confidences are also be returned
        [points,confidences] = self.get_most_confident_outputs(patch)

        pred_center = None
        first_close_visited = False
        if len(points) == 0:
            return None # end of this iteration
        else:

            prob_points = []
            # get the symmetry point of the estimated next center
            next_center_rev = np.array([2*seed[0]-next_center[0], 2*seed[1]-next_center[1]])
            for i in range(0, len(points)):
                # get the predicted road center point and its confidence inside the patch
                pt = points[i]
                confidence = confidences[i]
                # get the angle between one predicted road center points and the symmetry point of the estimated next center
            ...
            # sort the predicted road center points by the angle between the predicted road center point, seed and the symmetry point of the estimated next center
            ...
            # get the predicted road center point with the largest angle which far from the previous seed
            ...
            # below( the if clause) is to find the possible other road center points inside the patch, which will be treated as new seeds
            if pred_center is not None: 
                for prob_point in prob_points:
                    # the possible point is too close to the road line, neglected
                    if prob_point[1] * 180 / np.pi > 170: 
                        continue
                    # the possible point is too close to the found road center point, neglected
                    if self.get_len_from_two_points(pred_center, prob_point[0]) <= self.step_size / 5:
                        continue
                    # the possible point is too close to the tracked road, neglected
                    if self.get_min_distance(prob_point[0])[0] < self.close_visited_th:
                        continue
                    # the confidence of the possible point is too low, neglect
                    if prob_point[2] < np.max(confidences) * 0.5:
                        continue
                    # push the possible point into seed_stack as seed with initialized or uninitialized information
                    ret = False
                    if self.auto_init_seed:
            ...
            # if the model can not find a valid road center point, we can try to continue searching in the perpendicular direction
            # the possible seed in the perpendicular direction is pushed into the stack, which will be used in the following iteration
            if pred_center is None: 
                ...
                # whether the predicted point is too close to the visited road network, when the predicted point is too close
                # to the visited road for the first time, it doesn't matter, because the tracking road is crossing the visited roads.
                ...
                if min_dist <= self.close_visited_th: 
                    ...
                    # encounter the visited road twice successively, we regard it repeatly tracking the visited roads
                    if self.close_visited_time>1: 
                        return None
                    else:
                        # this is the first time it encounter the visited road
        ...
        # calculate the next center (along the forward direction)
        ...
        # clear the flag if it does not close to the visited roads
        # this is to realize the check of consecutive encountering visited roads
    ...
    def show_tracing(self):
        if self.visualize_tracing:
            if self.visualize_tracing_fig is None:
            ...
            path = np.array(self.visited_path[-1])
            plt.plot(path[:,1], path[:,0], color='red', linewidth=2, linestyle='-')
            plt.pause(0.1)

if __name__=="__main__":
    rt = road_tracking('iter_test4.png')
    rt.show_tracing()
    rt.auto_seed()

    while len(rt.seed_stack) > 0:
        m1 = time.time()
        ret = rt.next_iter()
        m2 = time.time()
        if ret:
            rt.show_tracing()
        m3 = time.time()
        print('predict time:{}, show time:{}'.format(m2-m1, m3 - m2))
    if messagebox.askokcancel('question','Finish tracking, do you want to save the tracking result?'):
        rt.save_results_fun()