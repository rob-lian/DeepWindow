...
from training.summaries import TensorboardSummary
...
dsname = 'mass'
db_root_dir = '../data/{}/'.format(dsname)
...
if p['useHist'] == 1:
    trans.append(tools.GrayHistEqual())
if p['useAug'] == 1:
    trans.append(tools.RandomFlip())

trans.append(tools.ToTensor())

composed_transforms = transforms.Compose(trans)

db_train = tools.ToolDataset(train=True, inputRes=p['inputRes'], outputRes=p['outputRes'],
...
modelName = tools.construct_name(p, model)
