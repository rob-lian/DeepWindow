...
class Zoom_Advanced(ttk.Frame):
    ...
    __bgcolor = 'green'
    __rectwidth = 3
    __fgtype = 1 #foreground code
    __bgtype = 0 #background code
    __rectcolor='yellow'
    __recttype = 2 #矩形框
    __scribble_show_finish_index = 0 #用于增量显示涂鸦
    __dragTags = None # 画布中有些对象可以被拖拽时使用
    ...
    def __init__(self, mainframe, path=None, patch_size=64):
        ...
        self.canvas.bind("<ButtonRelease-1> ", self.lrelease) #https://www.cnblogs.com/hhh5460/p/6701817.html
    ...
    def mouse_move(self, event ):
        ...
        item = self.canvas.gettags('current') # 返回(对象tags，'current')
        # the point marker (为了用户能够随时修改点标签的位置，如果是点便签，则可以让用户移动点标签位置）
    ...
    def setMouseMode0(self): #拖拽
        self.mouseMode = 0
    def setMouseMode5(self): #标记前景的点标签
        self.mouseMode = 5
    def setMouseMode6(self): #标记背景的点标签
        self.mouseMode = 6
    ...
    def move_to(self, event):
        ...
        elif self.mouseMode == 5 or self.mouseMode==6: # 拖动点标注
            # check the object at the current cursor, change the cursor according to the object type
            item = self.canvas.gettags('current') # 返回(对象tags，'current')
            # the point marker (为了用户能够随时修改点标签的位置，如果是点便签，则可以让用户移动点标签位置）
            if len(item)>1 and 'point-' in item[0]:
                ...
                self.__dragTags = item[0] #正在被拖动的点标签
        else:
            pass
    ...
    def lrelease(self, event):
        if self.mouseMode == 0:
            ...
            if self.__dragTags is not None:
                self.canvas.delete(self.__dragTags)
                self.removeScribbleByTags(self.__dragTags)
                self.__dragTags = None
            ...
            if self.mouseMode == 5:
                ...
                tags = 'point-back'
            point = [rx, ry]
            mark_t = Marker(point, 'o', color, 10, tags, len(self.allscribble))
    ...
    def show_grid(self):
        ...
        for r in range(0, bbox_height, intval_h):
            if (r+bbox_top+offset_y)<bbox_height:
                self.canvas.create_line(bbox_left, r+bbox_top+offset_y, bbox_left+bbox_width, r+bbox_top+offset_y, fill='gray', tags='grid_lines')

        for c in range(0, bbox_width, intval_w):
            if (c+bbox_left+offset_x)<bbox_width:
                self.canvas.create_line(c+bbox_left+offset_x, bbox_top, c+bbox_left+offset_x, bbox_height+bbox_top, fill='gray', tags='grid_lines')
    ...
    def addMarker(self, point, marker='o', color='yellow', thick=4, tags='marker', index=0):
        marker = Marker(point=point, marker=marker, color=color, thick=thick, tags=tags, index=index)
        self.allscribble.append(marker)
        ...
        for o in self.allscribble:
            if isinstance(o, Marker):
                ...
                if 'point-fore' in o.tags:
                    ...
                    # 搜寻是否存在其他标签属于相同切片的（即可能有些切片有两个路段，存在两个点标签）
                ...
                scipy.misc.imsave(os.path.join(save_dir, '%s_r%d_%04d_%04d_gt.png' % (imgid,have_road,left,top)), gt)
                np.save(os.path.join(save_dir , '%s_r%d_%04d_%04d_gt.npy' % (imgid,have_road,left,top)), gt)
                scipy.misc.imsave(os.path.join(save_dir , '%s_r%d_%04d_%04d_img.png' % (imgid,have_road,left,top)), img)

                chk = img.astype(np.int)
                chk[:,:,0] += (gt * 255).astype(np.int)
                chk[:,:,1] += (gt * 255).astype(np.int)
                chk[chk>255] = 255
                chk = chk.astype(np.uint8)
                scipy.misc.imsave(os.path.join(save_dir , '%s_r%d_%04d_%04d_chk.png' % (imgid,have_road,left,top)), chk)
